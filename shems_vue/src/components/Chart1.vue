<template>
  <div>
    <div class="chart-title">Energy Consumption Line Graph For a Specified Service Location in a Given Period of Time</div>
    <el-row>
    ...
    <el-row>
      <div id="myChart1" class="myChart1" style="width:1600px; height:800px"></div>
    </el-row>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    display() {
      ...
      let url = '';
      if (this.scope === 'Daily') {
        url = 'event/daily_energy_consumption';
        params = {
          slid: this.slid,
        ...
        url = 'event/monthly_energy_consumption';
        params = {
          slid: this.slid,
    ...
    showDailyGraph() {
      ...
      for (const [day, daily_energy_consumption] of Object.entries(rawData)) {
        const index = Math.floor((new Date(day) - start) / oneDay);
        yAxisData[index] = daily_energy_consumption;
      }
    ...
    showMonthlyGraph() {
      ...
      for (const [month, monthly_energy_consumption] of Object.entries(rawData)) {
        const date = new Date(month);
        const index = getMonthIndex(date);
        yAxisData[index] = monthly_energy_consumption;
      }
